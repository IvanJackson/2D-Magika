 ...
 */
public class SlimeKing extends CreatureBase  {
    ...
    public SlimeKing(Handler handler, float x, float y) {
        ...
        animDown = new Animation(animWalkingSpeed, Images.slime);
        animLeft = new Animation(animWalkingSpeed,Images.slime);
        animRight = new Animation(animWalkingSpeed,Images.slime);
        animUp = new Animation(animWalkingSpeed,Images.slime);
    ...
    private void checkIfMove() {
        ...
        if (SkelyCam.contains(handler.getWorld().getEntityManager().getPlayer().getX() - handler.getGameCamera().getxOffset(), handler.getWorld().getEntityManager().getPlayer().getY() - handler.getGameCamera().getyOffset())
            ...
            if (x >= handler.getWorld().getEntityManager().getPlayer().getX() - 10 && x <= handler.getWorld().getEntityManager().getPlayer().getX() + 10) {//nada
            ...
            if (y >= handler.getWorld().getEntityManager().getPlayer().getY() - 10 && y <= handler.getWorld().getEntityManager().getPlayer().getY() + 10) {//nada
                yMove = 0;
    ...
    public void render(Graphics g) {
        g.drawImage(getCurrentAnimationFrame(animDown,animUp,animLeft,animRight,Images.slime,Images.slime,Images.slime,Images.slime), (int) (x - handler.getGameCamera().getxOffset()), (int) (y - handler.getGameCamera().getyOffset()), width, height, null);
        if(isBeinghurt() && healthcounter<=120){
            g.setColor(Color.white);
            g.drawString("SlimeyHealth: " + getHealth(),(int) (x-handler.getGameCamera().getxOffset()),(int) (y-handler.getGameCamera().getyOffset()-20));
        }
    }
